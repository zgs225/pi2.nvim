...
A parent session can run **sub-sessions** in parallel — each child is an independent `pi --mode rpc` process with its own session file, model, and thinking level. Relationships and bookkeeping live in `<agent_dir>/sessions/<encoded-cwd>/.pi2-subsessions.json`; message content stays in each child's JSONL (the authoritative store). Child processes are **detached from the tab**, same as a parent after `:tabclose`: closing the parent tab does not stop running sub-sessions. Interrupt them with `:PiAbort`; close their processes with `:PiSubClose` / `x` in `:PiSessions`, `stop_subagents`, or by quitting Neovim. `:PiAbort` is scoped to the view you are in: issued on the parent, it interrupts the parent itself and cascades to all of its sub-sessions and their batches — including a batch the parent is currently waiting on, even one recorded under an older lineage — while every child that has already received its task keeps its RPC process alive (abort is not stop, so interrupted children can be sent new prompts or reused). Issued while viewing a child, it interrupts only that child — and any sub-sessions the child itself spawned with `:PiSubNew` — and is **not** forwarded to the parent: the parent's turn, its batches, and the sibling children all keep running undisturbed. What the parent does observe is accounting: the aborted child's pending batch items are settled immediately as `cancelled` (rather than left hanging or booked as `ok`), which wakes a parent blocked in `wait_subagents` right away instead of letting it hang until `subagent.batch_timeout_ms` — this settlement runs even when the child was already idle, where an abort produces no new agent events. A batch item aborted while its child is still spawning is the exception: that late child is aborted and its process reclaimed, since it never received a task and would otherwise linger as an orphan holding a `subagent.max_children` slot, whereas a child that already has its task is abort-only and keeps its process — an explicit close still only happens via `:PiSubClose` / `x` / `stop_subagents` / `Batch.cancel`, so no orphan process is left in either case.
...
1. **Explicit**: the `name` you pass on a `dispatch_subagents` `{ task }` item, or a name you actually type in the `:PiSubNew` dialog. Always wins and is **never overwritten**.
2. **Generated**: otherwise the child names *itself* — the bundled [auto session titles](#auto-session-titles) extension asks the child's model for a short title after its first turn and persists it. Because that is the same label a top-level session gets, an unnamed child ends up readable instead of being stuck at its task prefix.
3. **Derived**: until the generated title arrives (and whenever `title.enabled = false`), the label is the first 40 characters of the task.

The distinction between 1 and 2/3 is recorded in the manifest (`name_source`), so accepting the prefilled default in `:PiSubNew` stays replaceable while a typed name does not. Set `title = { enabled = false }` to opt out of generation — children then stay on the task prefix unless you name them.

Parent Agent tools are provided by the bundled `extensions/subagent.ts` extension when `subagent.enabled` is true. See [Extensions](extensions.md#bundled-sub-agent-extension-extensionssubagentts). Both sides also get a byte-constant **system-prompt note** per turn (`before_agent_start`): parents learn the orchestration discipline, children learn the worker contract (no interactive user, last message = final report, no questions, strict task scope, no nested sub-agents). Static text keeps pi's prompt-cache prefix stable.

**Agent workflow:** `list_subagents` to discover children (including dormant / completed after a parent resume) → `dispatch_subagents` for all work. Prefer `{ target, message }` when a matching child already exists — the host revives a closed process. Use `{ task }` only to spawn a new child, and give each new child a short descriptive `name` (2–5 words) — it is the label shown in `:PiSessions`, the dispatch block and the completion notice; omit it only for `{ target, message }` reuse items, which keep the existing child's name (see [Sub-session names](#sub-session-names)). When spawning with an explicit `model`, pi2.nvim validates model availability fail-fast: if the model is not found, spawn aborts and the item fails with an error listing available models (`Model not found: ... Available models: [...]`), enabling the agent to self-correct. `wait: true` on dispatch for one-shot results, or `poll_subagents` / `wait_subagents` with the returned `batch_id`. `stop_subagents` to close processes. Failure policy defaults to **collect_errors**. If the parent stays busy after children have finished, see [Troubleshooting → Sub-agent wait stuck](troubleshooting.md#sub-agent-wait-stuck-after-children-finish).
...
Keys inside the list: `<CR>` / `o` jump to that session's tab and open its chat (on a child row: bind the tab's chat to that sub-session), `a` / `i` do the same but drop you straight into Insert mode at the very end of that session's prompt draft (multi-line drafts land past the last line — ready to append), `r` renames the session under the cursor (same as `:PiSessionName`, without leaving the list), `s` shows the session's stats dashboard under the cursor (same data as `:PiSessionStats` — tokens, cost, context — in a dialog float, without leaving the list), `c` compacts the session under the cursor in the background (same as `:PiCompact` — the row's status dot switches to the compacting state while it runs, no tab switch), `d` opens the diff review of the session's changed files under the cursor (same panel as `:PiDiff` — the combined git diff in a floating window, without leaving the list), `f` / `C` / `t` fork, clone, or tree-navigate the session under the cursor (same flows as `:PiFork` / `:PiClone` / `:PiTree` — the list jumps to that session's tab first, so the pickers, prefill and streaming guards all run against the right session), `p` previews the session under the cursor — a parent/tab row or a sub-session child row — in the same read-only float viewer with rendered chat history, the session's display name as the title, and a statusline in the footer (context tokens/window, model, thinking level, and a `✓`/`◐`/`○` todo summary when the viewed session has todos; no tab switch): live sessions stream in real time, sessions whose process is gone replay from that session's JSONL file on disk, and inside the viewer `<CR>` promotes a sub-session child row into the tab's chat, while on a parent/tab row it closes the viewer and jumps to that session's tab with its chat prompt focused (warning instead if the session's process is not running), `T` inside the viewer toggles the viewed session's todo list panel, `x` closes a sub-session's RPC process, `<Tab>` / `za` fold or unfold sub-sessions under a parent (on a child row: collapse its parent and jump to it), `zM` / `zR` collapse or expand all sub-sessions, `H` toggles hidden sub-session rows globally (dormant / settled, every session at once), `h` does the same for the session under the cursor only — on a child row it toggles that child's parent (the subagent's own rows live under its parent), `R` re-fetches session names, `?` toggles a shortcut help overlay, `q` closes the window.
...
require("pi").setup({
    sessions_list = {
        ...
        collapse_subsessions = false, -- collapse sub-sessions by default
