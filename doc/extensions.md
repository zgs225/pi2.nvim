...
| `dispatch_subagents` | Fan out parallel work (mixed `{ task }` spawn + `{ target, message }` reuse). Dormant/settled ids still work — the host revives the process. Optional `wait: true` blocks until done. A `{ task }` item takes an optional `name` (short label for the child, see [Sessions → Sub-session names](sessions.md#sub-session-names)). |
