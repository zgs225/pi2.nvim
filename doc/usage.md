...
Three commands are handled locally instead of being sent: a bare `/tree` opens the [session tree navigator](sessions.md#session-tree-navigation-pitree) (`:PiTree`), a bare `/fork` opens the [fork picker](sessions.md#fork-and-clone) (new session from a past user message), and a bare `/clone` duplicates the current branch into a new session — mirroring the TUI's built-in `/tree`, `/fork`, and `/clone`.
...
### In the sub-session viewer

The read-only float viewer (`p` on a row in the [sessions overview](sessions.md#sessions-overview-pisessions), or `:PiSubView`) surfaces the *viewed* session's todos as well, gated by `subagent.viewer.todo` (`enabled`, `auto_open`, `position`, `height`, `max_items` — see [Configuration](configuration.md)). While the list is non-empty, the viewer's footer statusline gains a summary chunk after the context / model / thinking components: `✓ 5/5` once every item is completed, `◐ 2/5` while an item is in progress, `○ 0/5` when nothing has started yet — the same ✓/◐/○ three states as the checklist above, colored with [`PiTodoDone` / `PiTodoInProgress` / `PiTodoPending`](highlight-groups.md#sub-session-viewer). With an empty list, or `subagent.viewer.todo.enabled = false`, the footer stays exactly as it was without the feature — and since the chunk rides in the footer, `subagent.viewer.statusline = false` hides it too.

`T` inside the viewer toggles a todo panel anchored to the bottom-right corner of the viewer float (`subagent.viewer.todo.position = "above"` hangs it off the top edge instead, still aligned to the right): the progress header plus the checklist rendered like the `:PiTodo` panel, capped at `subagent.viewer.todo.max_items` items with a `… N more` tail, and sized to its content with an inner padding of one blank line above and below plus one blank column on the left — its height up to `subagent.viewer.todo.height` (a fraction of the viewer height, default `0.35`) and its width at most 60% of the viewer width, longer items clipped rather than wrapped since the panel never wraps. The panel never takes focus — your cursor and current window stay put — and an empty list shows a `no todos` placeholder instead of refusing to open. For a live session, the footer chunk and an open panel both refresh **in place** on every `todo_write` result (the panel repaints and re-fits without moving); a session whose process is gone shows the last list found in its JSONL file as of when the viewer opened, and stays static until you reopen the viewer.

With `subagent.viewer.todo.auto_open`, the panel opens itself when the viewed live session's list transitions from empty to non-empty (the first write while you watch, or a re-added list after a clear — opening the viewer on a session that already has todos does not pop it). Closing the panel with `T` suppresses that auto-open until you press `T` again, and closing the viewer tears the panel down with it (without leaving suppression behind). `enabled = false` drops the footer chunk and makes `T` warn instead of opening the panel.
