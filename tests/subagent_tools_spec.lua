...
describe("subagent tool icons", function()
    ...
    it("dispatch_subagents has no collapse thresholds (stays expanded)", function()
        local renderer = Tools.get_renderer("dispatch_subagents")
        assert.is_nil(renderer.input_visible, "expected nil input_visible (unlimited)")
        assert.is_nil(renderer.output_visible, "expected nil output_visible (unlimited)")
    end)
end)
