...
describe("vision", function()
    ...
    describe("publish / published", function()
        after_each(function()
            Vision._set_path(nil)
        end)

        it("round-trips the configured model through the runtime file", function()
            Vision._set_path(vim.fn.tempname())
            Vision.publish("kimi-coding/kimi-for-coding")
            assert.are.equal("kimi-coding/kimi-for-coding", Vision.published())
        end)

        it("clears the publication when the model is unset", function()
            Vision._set_path(vim.fn.tempname())
            Vision.publish("p/m")
            Vision.publish(nil)
            assert.is_nil(Vision.published())
            Vision.publish("")
            assert.is_nil(Vision.published())
        end)
    end)
