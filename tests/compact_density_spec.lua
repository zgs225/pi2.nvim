...
describe("compact density", function()
    ...
    describe("spacing", function()
        ...
        it("starts text on a fresh line directly below the agent label", function()
            ...
            pump()
            local lines = lines_of(h)
            assert.are_equal(Config.options.labels.agent_response, lines[1])
            assert.are_equal("reply", (lines[2]:match("^reply")))
        end)
    end)
