...
describe("pi.title", function()
    ...
    describe("published", function()
        ...
        it("round-trips a pinned generation model", function()
            Title.publish({ enabled = true, max_chars = 25, lang = "zh-CN", model = "openai/gpt-4o-mini" })
            local cfg = Title.published()
            assert.are.equal("openai/gpt-4o-mini", cfg.model)
            Title.publish({ enabled = true, model = "" })
            assert.is_nil(Title.published().model)
        end)
